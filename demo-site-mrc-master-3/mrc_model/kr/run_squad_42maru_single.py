...
class SquadExample(object):
    ...
    def __repr__(self):
        ...
        s += ", orig_answer_text: [%s]" % (tokenization.printable_text(self.orig_answer_text))
    ...
    for entry in input_data:
        for paragraph in entry["paragraphs"]:
            ...
            for qa in paragraph["qas"]:
                ...
                is_impossible = None
                if is_training:
                    if len(qa["answers"]) != 1:  # 트레이닝의 경우, 정답은 반드시 하나!
                        is_impossible = 1
                        # if qa["plausible_answers"]:
                        answer = qa["plausible_answers"][0]
                        # else:
                        # print('do not have any answers')
                        # continue
                        '''raise ValueError(
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        all_predictions[example.qas_id] = '' if - (
        nbest_json[0]["is_ans"] + nbest_json[0]["is_ans_max"] + nbest_json[0]["has_ans_max"]) > FLAGS.thresh else \
        nbest_json[0]["text"]
        # all_predictions[example.qas_id] = nbest_json[0]["text"]
        all_predictions_pb[example.qas_id] = - (
        nbest_json[0]["is_ans"] + nbest_json[0]["is_ans_max"] + nbest_json[0]["has_ans_max"])
